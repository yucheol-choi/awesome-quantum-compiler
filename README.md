...
How to read this reporatory.
	1.	Front-end IR is where language and loop constructs live; everything else either optimises, verifies, or realises that IR.
	2.	Middle-end is logic-level optimisation plus gate/cycle scheduling.
	3.	Back-end/control turns scheduled circuits into hardware-runnable pulse streams, strongly shaped by the device’s ISA.
	4.	Verification and Benchmark/Sim/Test aren’t single pipeline stages—they are orthogonal services that consume artefacts from several layers to prove correctness or measure performance.
...
+ Research Papers
  ...
  + Front-end & High-Level IR
    ...
    + Circuit Analysis (static metrics, cost models)
  + Middle-end Optimisations & Scheduling
    + Circuit Scheduling (gate/cycle level)
    + Gate/Cycle Scheduling. Logic-level (depth, T-count, synthesis)
    + Layout-level passes (mapping, SWAP, connectivity, partitioning)  
  + Back-end & Hardware Execution / Runtime
    + Cycle/Gate Scheduling (ISA layer)
    + Control-level (pulse scheduling, optimal control)
    + Runtime & ISA (Hardware-aware compilation, Quantum ISA)
  + Veri & Bench / Simul & Debug 
    + Verification & Benchmark
    + Evaluation & Simulation 
    + Debug & Test Quantum IR
...
4. Runtime & ISA (Hardware-aware compilation, Quantum ISA)

#### Hardware Execution & Runtime Scheduler (Back-end / Control stack)
...
6. Error Mitigation and Correction
